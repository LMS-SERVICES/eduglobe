...
export default function TakeMockTestPage() {
  ...
  }, [secondsLeft, test, result, submitting])
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          <div className="bg-white rounded-xl border border-slate-200 p-6 text-center">
            ...
            <div className="mt-4 flex justify-center gap-3">
              ...
              <button onClick={() => { setResult(null); setAnswers({}); setSecondsLeft((test.durationMinutes || 0) * 60) }} className="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary-light">Re-attempt</button>
            </div>
          </div>
        ...
        <aside className="lg:col-span-1 bg-white border border-slate-200 rounded-xl p-4 h-fit">
          ...
          <div className="space-y-2">
            {test.sections.map((sec, idx) => {
              ...
              return (
                <button key={sec.id} onClick={() => setActiveSection(idx)} className={`w-full text-left px-3 py-2 rounded-lg border ${activeSection === idx ? 'border-primary bg-primary/5 text-primary' : 'border-slate-200 hover:bg-slate-50 text-slate-700'}`}>
          ...
          <Link href={`/mock-test/${id}`} className="block mt-4 text-sm text-slate-500 hover:text-slate-700">Exit test</Link>
        </aside>
        ...
        <main className="lg:col-span-3 space-y-4">
          ...
          {currentQuestions.map((q, qi) => (
            <div key={q.id} className="bg-white border border-slate-200 rounded-xl p-5">
              ...
              <p className="text-xs text-slate-500 mt-1">Marks: +{q.marks} {q.negativeMarks ? `| Negative: -${q.negativeMarks}` : ''}</p>
              <div className="mt-3 space-y-2">
                {q.options.map((opt, oi) => (
                  <label key={opt.id} className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer ${answers[q.id] === opt.id ? 'border-primary bg-primary/5' : 'border-slate-200 hover:bg-slate-50'}`}>
                    <input
                      type="radio"
                      name={q.id}
